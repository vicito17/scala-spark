...
object MostPopularSuperhero {
  ...
  def main(args: Array[String]) {
   ...
   // println(s"$mostPopularName is the most popular superhero with ${mostPopular._1} co-appearances.")
    ...
    val desc=flipped.sortByKey(false)

    var j=0
    for(i <- desc.take(1)){
      j=j+1
      println(s"${namesRdd.lookup(i._2)(0)} ostenta la posición $j entre los mas populares con  ${i._1} coapariciones.")

    }
    val asce=flipped.sortByKey(true)
    j=0
    for(i <- asce.take(10)){
      j=j+1
      println(s"${namesRdd.lookup(i._2)(0)} ostenta la posición $j entre los menos populares con  ${i._1} coapariciones.")
  }
  }
}
